#lang scheme/gui
; export values
; make alarm (visual) if butterfly goes out (disappear and flash?) butterfly burns up...
(require lang/prim
         lang/posn
...
(define butterfly (bitmap "teachpack-images/butterfly-02.png"))
(define kuyu (bitmap "teachpack-images/kuyu.png"))
(define bahçe (bitmap "teachpack-images/garden.png"))
...
;                    (draw-text w (empty-scene WIDTH HEIGHT)))))
                    (draw-text w (overlay/align "middle" "middle" kuyu bahçe)))))
  ...
  (let* ((onscreen?* (if (= (procedure-arity onscreen?) 2) 
         ...
         (update (lambda (w k) 
                   (if (onscreen?* (world-x (move w k)) 
                       ...
                       (move w k)
                       w))))
    (big-bang (make-world (+ (/ WIDTH 2) 100) (+ (/ HEIGHT 2) 100))
              (on-draw draw-world)
              (on-key update))))
