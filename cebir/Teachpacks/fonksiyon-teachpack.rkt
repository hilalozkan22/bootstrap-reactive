...
(provide-higher-order-primitive uzay (rocket-height))
...
(define (legend w)
  (let* ((time (car w))
         ...
         (ttime (beside (time-text
                         (string-append "Time: " (number->string time) (if (= 1 time) " saniye" " saniye")))
                        (scale 1.2 (time-text "       [boşluk tuşu] bir saniye ekler.  [b] dönüyor!"))))
         (theight (height-text 
                   (string-append "Yükseklik: " (number->string height) (if (= 1 height) " metre" " metre"))))
         (tspeed (speed-text 
                  (string-append "Hız: " (number->string speed) (if (= 1 speed) " metre/saniye" " metre/saniye")
